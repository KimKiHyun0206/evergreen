# 📅 2026-02 Learning Log
이번 달의 성장을 기록합니다. 🌱


<details><summary><b>🌱 2026-02-01 (Sun)</b></summary>

- AWS CDK 핵심 개념 학습: Bootstrap, Aspects.
</details>

<details><summary><b>🌱 2026-02-02 (Mon)</b></summary>

- AWS CDK 핵심 개념 학습: Context & Parameters, Assets.
</details>

<details><summary><b>🌱 2026-02-03 (Tue)</b></summary>

- [AWS CDK 합성과 배포 학습](https://velog.io/@duskafka/AWS-CDK-%ED%95%A9%EC%84%B1%EA%B3%BC-%EB%B0%B0%ED%8F%AC)
</details>

<details><summary><b>🌱 2026-02-04 (Wed)</b></summary>

- **AWS CDK 학습**: `cdk diff`와 `HotSwap`에 대해서 학습.
</details>

<details><summary><b>🌱 2026-02-05 (Thu)</b></summary>

- **AWS CloudFormatiion 학습**: AWS CDK의 동작을 이해하기 위해 학습함.
- AWS CDK Rollback 학습.
</details>

<details><summary><b>🌱 2026-02-06 (Fri)</b></summary>

- 휴식
</details>

<details><summary><b>🌱 2026-02-07 (Sat)</b></summary>

- AWS CDK 코딩
</details>

<details><summary><b>🌱 2026-02-08 (Sun)</b></summary>

- AWS CDK 코딩
</details>

<details><summary><b>🌱 2026-02-09 (Mon)</b></summary>

- 휴식
</details>

<details><summary><b>🌱 2026-02-10 (Tue)</b></summary>

- 휴식
</details>

<details><summary><b>🌱 2026-02-11 (Wed)</b></summary>

- **AWS CDK 설계**: AWS 인스턴스와 클래스를 1:1로 매핑시킨다면 클래스가 늘어나고 더 복잡해지고, AWS Constuct L1 레벨의 클래스를 사용해야 한다는 것을 알게됨. 이로 인해 객체지향의 특징을 살려 1:1로 매핑하는 것에 대해서 더 신중하게 고민해볼 필요성을 느낌.
</details>

<details><summary><b>🌱 2026-02-12 (Thu)</b></summary>

- **Aetheria 프로젝트 리프레시 토큰 블랙리스트 정책 수정**: 
</details>

<details><summary><b>🌱 2026-02-13 (Fri)</b></summary>

- **Aetheria 프로젝트 보안 정책 문서화:**
    1. 액세스 토큰 암호화
    2. 리프레시 토큰 로테이션 정책
    3. 데이터베이스 및 액세스 토큰 페이로드 암호화
    4. 리프레시 토큰 기반 세션 관리 정책
- **Aetheria 프로젝트 Limit Rate 기능 구현**: Redis와 Lua Script를 사용하여 구현 (테스트는 아직)
</details>

<details><summary><b>🌱 2026-02-14 (Sat)</b></summary>

- 휴식
</details>

<details><summary><b>🌱 2026-02-15 (Sun)</b></summary>

- **Rate Limit 기능 구현**: Redis를 사용하여 토큰 버킷 형식으로 구현함.
</details>

<details><summary><b>🌱 2026-02-16 (Mon)</b></summary>

- **Rate Limit 기능 구현 완료**: 코드 리펙토링 및 구현, 테스트 코드 작성 완료.
</details>

<details><summary><b>🌱 2026-02-17 (Tue)</b></summary>

- 휴식
</details>

<details><summary><b>🌱 2026-02-18 (Wed)</b></summary>

- rate limit PR 완료
</details>

<details><summary><b>🌱 2026-02-19 (Thu)</b></summary>

- 휴식
</details>

<details><summary><b>🌱 2026-02-20 (Fri)</b></summary>

- 마인크래프트 모드 구현
</details>

<details><summary><b>🌱 2026-02-21 (Sat)</b></summary>

- 마인크래프트 모드 구현 및 리펙토링
</details>

<details><summary><b>🌱 2026-02-22 (Sun)</b></summary>

- 마인크래프트 모드 구현 및 리펙토링
</details>

<details><summary><b>🌱 2026-02-23 (Mon)</b></summary>

- 마인크래프트 모드 구현 및 리펙토링
</details>

<details><summary><b>🌱 2026-02-24 (Tue)</b></summary>

- 
</details>

<details><summary><b>🌱 2026-02-25 (Wed)</b></summary>

- 
</details>
